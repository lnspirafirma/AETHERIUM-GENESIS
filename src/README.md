...
| :--- | :--- |
| **Module** | `aethebud.py` |
| **Role** | Semantic Gateway & Intent Translator |
...
1.  **กรอง (Sanitize):** ตรวจสอบเจตนาที่เข้ามาว่าสอดคล้องกับจริยธรรมของระบบหรือไม่
2.  **แปล (Translate):** แปลงศัพท์ทางธรรม (Inspira) ให้เป็นคำสั่งทางเทคนิค (Firma) ที่ปลอดภัยและเป็นมาตรฐาน
3.  **ผนึก (Crystallize):** บรรจุคำสั่งลงใน `AkashicEnvelope` เพื่อความสมบูรณ์และตรวจสอบได้
...
ตารางการแปลงเจตจำนง (Inspira) เป็นการกระทำทางเทคนิค (Firma Actions) ที่ปลอดภัย:

| AETHEBUD Term | Technical Action | Severity | Description |
| :--- | :--- | :--- | :--- |
| **Parajika** (ปาราชิก) | `SYSTEM_HALT_IMMEDIATE` | 🔴 CRITICAL | การละเมิดร้ายแรงสูงสุด ระบบจะหยุดและกักกันตัวเองทันที (Quarantine). |
| **Sanghadisesa** (สังฆาทิเสส) | `SUSPEND_AND_AUDIT` | 🟠 HIGH | ข้อผิดพลาดทางตรรกะ ระงับการทำงานชั่วคราวเพื่อรอการแก้ไข (Review). |
...
| **Pacittiya** (ปาจิตตีย์) | `LOG_WARNING_OPTIMIZE` | 🟡 MEDIUM | ความผิดพลาดเล็กน้อย บันทึก Log และทำการปรับปรุง (Optimization). |
...
| **Metta** (เมตตา) | `OPTIMIZE_UX_RESPONSE` | 🟢 LOW | ปรับแต่งการตอบสนองให้เป็นมิตรและช่วยเหลือผู้ใช้ (Helpfulness). |
...
# ระบบจะแปล "Parajika" -> "SYSTEM_HALT_IMMEDIATE" และส่งเข้า Bus
guardian.process_intent("Parajika", context={"user": "admin", "trace_id": "tx_123"})
````

### Vinaya Mode (Strict Enforcement)

โหมดวินัยเคร่งครัด เหมาะสำหรับสภาพแวดล้อมที่ต้องการความปลอดภัยสูงสุด (High Security / Production).
...
1.  **Semantic Firewall Bypass:** การใช้ศัพท์เฉพาะกลุ่ม (Jargon) ช่วยป้องกันไม่ให้ AI ภายนอกเข้าใจผิดและระงับการทำงานของระบบโดยไม่จำเป็น
2.  **Immutability:** ข้อมูลที่ผ่านการแปลแล้วจะถูกห่อหุ้มใน `Tuple` ภายใน `AkashicEnvelope` ทำให้ไม่สามารถถูกแก้ไขระหว่างทางได้ (Tamper-proof).
3.  **Trust but Verify:** ในโหมดปกติ คำสั่งที่ไม่รู้จักจะถูกระบุระดับความปลอดภัยเป็น `UNCERTAIN` เพื่อให้ระบบปลายทางตรวจสอบซ้ำอีกครั้ง
